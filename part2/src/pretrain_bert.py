"""
BERT-like model pretraining with Masked Language Modeling (MLM).
...
os.makedirs('outputs/bert_pretrain', exist_ok=True)
os.makedirs('logs',                  exist_ok=True)

# ── Dataset ───────────────────────────────────────────────────────────────────
full_dataset = MLMDataset(
...
model = BERTModel(
    ...
    num_kv_heads = config.get('num_kv_heads',  12),
...
print(f"  BERT params : {total:,}  (~{total/1e6:.1f}M)")
...
for epoch in range(config['epochs']):
    ...
    for step, batch in enumerate(bar):
        ...
        pad_mask  = (input_ids == 0)  # True = padding

        with autocast('cuda', dtype=AMP_DTYPE):
            logits = model(input_ids, padding_mask=pad_mask)           # (B, T, V)
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader, desc="Validating", leave=False):
            ...
            with autocast('cuda', dtype=AMP_DTYPE):
                logits = model(input_ids, padding_mask=pad_mask)
                loss   = criterion(logits.view(-1, config['vocab_size']), labels.view(-1))
            val_loss += loss.item()
    ...
    torch.save(model.state_dict(), f"checkpoints/bert_pretrain_epoch{epoch+1}.pt")
    if val_loss < best_val_loss:
        best_val_loss = val_loss
        best_epoch    = epoch + 1
        torch.save(model.state_dict(), "checkpoints/bert_pretrained_best.pt")
        print(f"    ✓  Best BERT model saved (epoch {best_epoch})")
...
metrics_df.to_csv('outputs/bert_pretrain/metrics.csv', index=False)

plot_metric(train_losses, val_losses, 'Loss (MLM)',
            'outputs/bert_pretrain/loss_plot.png', title='BERT MLM Train vs Val Loss')
plot_single([math.exp(min(l,20)) for l in train_losses],
            'Perplexity', 'outputs/bert_pretrain/train_ppl_plot.png', 'Train Perplexity')
plot_single([math.exp(min(l,20)) for l in val_losses],
            'Perplexity', 'outputs/bert_pretrain/val_ppl_plot.png', 'Val Perplexity')
plot_lr(lr_history, 'outputs/bert_pretrain/lr_plot.png')
...
with open('outputs/bert_pretrain/training_summary.json', 'w') as f:
    json.dump(summary, f, indent=2)

print("\n" + "=" * 60)
print(f"  BERT pretraining complete  |  {total_h:.2f} h")
print(f"  Best epoch: {best_epoch}  |  Best val loss: {best_val_loss:.4f}")
print(f"  Checkpoint: checkpoints/bert_pretrained_best.pt")
print("=" * 60)
