...
print("=" * 60)
print("  Random Embeddings — NMT Training")
print("=" * 60)
...
# ── Model (randomly initialized embeddings) ───────────────────────────────────
...
for epoch in range(config['epochs']):
    ...
    # ── Checkpoint ────────────────────────────────────────────────────────
    torch.save(model.state_dict(), f"checkpoints/random_epoch_{epoch+1}.pt")

    if val_loss < best_val_loss:
        best_val_loss = val_loss
        best_epoch    = epoch + 1
        torch.save(model.state_dict(), "checkpoints/best_random_model.pt")
        with open("outputs/random/best_val_predictions.txt", "w", encoding="utf-8") as f:
...
metrics_df.to_csv("outputs/random/metrics.csv", index=False)

# ── Plots ─────────────────────────────────────────────────────────────────────
plot_metric(train_losses,      val_losses,      "Loss",        "outputs/random/loss_plot.png",
            title="Train vs Validation Loss")
plot_metric(train_bleu_scores, val_bleu_scores, "BLEU (100)",  "outputs/random/bleu_plot.png",
            title="Train vs Validation BLEU (100)")
plot_metric(train_chrf_scores, val_chrf_scores, "CHRF++ (100)","outputs/random/chrf_plot.png",
            title="Train vs Validation CHRF++ (100)")
plot_lr(lr_history, "outputs/random/lr_plot.png")
...
model.load_state_dict(torch.load("checkpoints/best_random_model.pt", map_location=DEVICE))
...
with open("outputs/random/test_results.txt", "w", encoding="utf-8") as f:
    f.write(f"Best epoch       : {best_epoch}\n")
    f.write(f"Test Loss        : {test_loss:.4f}\n")
    f.write(f"Test BLEU (100)  : {test_bleu:.2f}\n")
    f.write(f"Test CHRF++ (100): {test_chrf:.2f}\n")

with open("outputs/random/test_predictions.txt", "w", encoding="utf-8") as f:
...
with open("outputs/random/training_summary.json", "w", encoding="utf-8") as f:
    json.dump(summary, f, indent=2, ensure_ascii=False)

print("\n" + "=" * 60)
print(f"  Random training complete  |  total time: {total_time_h:.2f} h")
print(f"  All results saved to outputs/random/")
print("=" * 60)
